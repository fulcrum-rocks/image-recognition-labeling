...
class DetectionModelTrainer:
    ...
    def setDataDirectory(self, data_directory):
        ...
        self.__train_images_folder = os.path.join(
            data_directory, "train", "images")
        self.__train_annotations_folder = os.path.join(
            data_directory, "train", "annotations")
        self.__validation_images_folder = os.path.join(
            data_directory, "validation", "images")
        self.__validation_annotations_folder = os.path.join(
            data_directory, "validation", "annotations")

        os.makedirs(os.path.join(data_directory, "cache"), exist_ok=True)
        self.__train_cache_file = os.path.join(
            data_directory, "cache", "detection_train_data.pkl")
        self.__validation_cache_file = os.path.join(
            data_directory, "cache", "detection_test_data.pkl")
        ...
        self.__train_weights_name = os.path.join(
            self.__model_directory, "detection_model-")
    ...
    def setTrainConfig(self,  object_names_array, batch_size=4, num_experiments=100, train_from_pretrained_model=""):
        ...
        print("Detection configuration saved in ", os.path.join(
            self.__json_directory, "detection_config.json"))
    ...
    def evaluateModel(self, model_path, json_path, batch_size=4, iou_threshold=0.5, object_threshold=0.2, nms_threshold=0.45):
        ...
        if os.path.isfile(model_path):
            ...
            model_files = sorted([os.path.join(model_path, file_name)
                                  for file_name in os.listdir(model_path)])
            # sort the files to make sure we're always evaluating them on same order
        else:
            print('model_path must be the path to a .h5 file or a directory. Found {}'.format(
                model_path))
            return results
            ...
            if str(model_file).endswith(".h5"):
                try:
                    ...
                    for label, average_precision in average_precisions.items():
                        ...
                        result_dict['average_precision'][labels[label]
                                                         ] = average_precision
                    print('mAP: {:.4f}'.format(
                    ...
                    print('skipping the evaluation of {} because following exception occurred: {}'.format(
                        model_file, e))
                    continue
            else:
                print(
                    'skipping the evaluation of {} since it\'s not a .h5 file'.format(model_file))
        return results
    ...
    def _create_callbacks(self, saved_weights_name, model_to_save):
        ...
        checkpoint = CustomModelCheckpoint(
            ...
            'ex-{epoch:03d}--loss-{loss:08.3f}.h5',
            monitor='loss',
            verbose=0,
            save_best_only=True,
            mode='min',
...
class CustomObjectDetection:
    ...
    def loadModel(self):
        ...
        if self.__model_type == "yolov3":
            detection_model_json = json.load(
                open(self.__detection_config_json_path))

            self.__model_labels = detection_model_json["labels"]
            self.__model_anchors = detection_model_json["anchors"]

            self.__detection_utils = CustomDetectionUtils(
                labels=self.__model_labels)

            self.__model = yolo_main(
                Input(shape=(None, None, 3)), 3, len(self.__model_labels))
        ...
        if self.__model is None:
            ...
            if output_type == "file":
                ...
                output_image_folder, n_subs = re.subn(
                    r'\.(?:jpe?g|png|tif|webp|PPM|PGM)$', '', output_image_path, flags=re.I)
                if n_subs == 0:
            ...
            if input_type == "file":
                ...
                raise ValueError(
                    "input_type must be 'file' or 'array'. {} found".format(input_type))
            ...
            if self.__model_type == "yolov3":
                ...
                self.__detection_utils.correct_yolo_boxes(
                    boxes, height, width, self.__input_size, self.__input_size)
                ...
                for object_box, object_label, object_score in zip(all_boxes, all_labels, all_scores):
                    ...
                    each_object_details["box_points"] = [
                        object_box.xmin, object_box.ymin, object_box.xmax, object_box.ymax]
...
class CustomDetectionUtils:
        ...
        for row in range(grid_h):
            for col in range(grid_w):
                for b in range(nb_box):
                    ...
                    y = (row + y) / grid_h
                    w = anchors[2 * b + 0] * \
                        np.exp(w) / net_w  # unit: image width
                    h = anchors[2 * b + 1] * \
                        np.exp(h) / net_h  # unit: image height
                    # last elements are class probabilities
                    classes = netout[row, col, b, 5:]
                    box = BoundBox(x - w / 2, y - h / 2, x +
                                   w / 2, y + h / 2, objectness, classes)
                    boxes.append(box)
    ...
    def correct_yolo_boxes(boxes, image_h, image_w, net_h, net_w):
        ...
        for i in range(len(boxes)):
            x_offset, x_scale = (net_w - new_w) / 2. / \
                net_w, float(new_w) / net_w
            y_offset, y_scale = (net_h - new_h) / 2. / \
                net_h, float(new_h) / net_h
    ...
    def draw_boxes_and_caption(self, image_frame, v_boxes, v_labels, v_scores, show_names=False, show_percentage=False):
        ...
        for i in range(len(v_boxes)):
            ...
            image_frame = cv2.rectangle(
                image_frame, (x1, y1), (x2, y2), class_color, 2)
            ...
            if show_names or show_percentage:
                ...
                cv2.putText(
                    image_frame, label, (b[0], b[1] - 10), cv2.FONT_HERSHEY_PLAIN, 1, (200, 0, 0), 3)
                cv2.putText(
                    image_frame, label, (b[0], b[1] - 10), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2)
